...
exports.onUpdateFunction = functions.database.ref('Maintodo/{dataId}').onUpdate((change, context) => {
    ...
    ref.set({update: updatedData.update ? previousData.update + 1 : 0})
    .then(() => {console.log(`Data ${dataId} updated successfully.`); })
    .catch((error) => {console.error(`Error updating data ${dataId}:`, error);});
});